...
Authors@R: c(
	...
	person(given = "${author[0]}",
<#if author[1]??>		family = "${author[1]}",
</#if><#if author[2]??>		email = "${author[2]}",
</#if><#if author[3]??>		comment = structure("${author[3]}", .Names = "ORCID"),
</#if>		role = <#if author?is_first>c("aut", "cre")<#else>"aut"</#if>)<#sep>,
	</#list>
)
...
<#list model.getImports() as import>	${import},${"\n"}</#list>	rJava,
	...
	xml2,
	stringr
<#if (model.getConfig().getUrls()?size>0)>
URL: ${model.getConfig().getUrls()?join(",\n\t")}
</#if>
<#if model.getConfig().getGithubOrganisation()?? && model.getConfig().getGithubRepository()??>
BugReports: https://github.com/${model.getConfig().getGithubOrganisation()}/${model.getConfig().getGithubRepository()}/issues
</#if>